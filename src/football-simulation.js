...
class FootballSimulation extends Component {
	...
	move = (step) => {
		var positions = this.playerPositionsByTick[step];
	...
	playByIntervalFn = (step) => () => {
		...
		if (step === Object.keys(this.playerPositionsByTick).length - 1) {
	...
	componentDidMount() {
		this.playerPositionsByTick[0].forEach((position) => {
